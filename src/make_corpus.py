...
def initial_relabel(g, node_label_attr_name="Label"):
    ...
    nx.convert_node_labels_to_integers(
        g, first_label=0
    )  # this needs to be done for the initial interation only
    for node in g.nodes():
    ...
    for node in g.nodes():
        ...
        if not node_id in node_label_map:
            ...
            node_label_map[node_id] = (
                node_label  # insert the new label to the label map
            )
...
def wl_relabel(g, it, debug=False):
    ...
    try:
        ...
        new_g = deepcopy(g)
        for n in g.nodes():
            new_g.nodes[n]["neighborhood_label"] = ast.literal_eval(
                g.nodes[n]["neighborhood_label"]
            )
        g = new_g
    except:
        opfname = None
    ...
    for node in g.nodes():
        ...
        neighborhood_label = sorted(
            [g.nodes[nei]["neighborhood_label"][prev_iter] for nei in neighbors]
        )

        new_node_features = [
            g.nodes[node]["neighborhood_label"][prev_iter]
        ] + neighborhood_label
        ...
        g.nodes[node]["neighborhood_label"][
            it
        ] = next_label  # get_hash(new_node_features)
...
def write_labels_to_file(
    label, legend_tag, fh_vocabulary, fh_vocabulary_legend, word_tag
):
    print(str(label) + "#" + word_tag, file=fh_vocabulary)
    label_legend = add_label_tag(str(label) + "#" + word_tag, legend_tag)
    print(label_legend, file=fh_vocabulary_legend)


def dump_sg2vec_str(fname, g, vocabulary_params, word_tag):
    ...
    for n, d in g.nodes(data=True):
        ...
        if vocabulary_params["neighborhoods"]:
            for i in d["neighborhood_label"].keys():  # TODO (this is redundant).
                ...
                if i not in vocabulary_params["wlk_sizes"]:
                    continue
                if (
                    vocabulary_params["remove_unique_words"]
                    and word_counts[d["neighborhood_label"][i]] <= 2
                ):
                ...
                if i == 0:  # individual nodes
                    if d["neighborhood_label"][i] != str(RELABELED_ROOT_LABEL):
                        write_labels_to_file(
                            d["neighborhood_label"][i],
                            "single_node",
                            fh_vocabulary,
                            fh_vocabulary_legend,
                            word_tag,
                        )
        ...
        if vocabulary_params["structure"]:
            for i in d["structure_label"].keys():
                ...
                if (
                    vocabulary_params["remove_unique_words"]
                    and word_counts[d["structure_label"][i]] <= 2
                ):
        ...
        if vocabulary_params["individual_nodes"]:
            if str(d["neighborhood_label"][0]) != str(RELABELED_ROOT_LABEL) and str(
                d["neighborhood_label"][0]
            ) != str(RELABELED_LABEL_TO_IGNORE):
                if (
                    vocabulary_params["remove_unique_words"]
                    and word_counts[d["neighborhood_label"][0]] <= 2
                ):
                    unique_word = d["neighborhood_label"][0]
                else:
                    has_non_unique_words = True
                    for label_copy in multiply_label(
                        vocabulary_params["individual_nodes"],
                        d["neighborhood_label"][0],
                    ):
                        write_labels_to_file(
                            label_copy,
        ...
        if vocabulary_params["non_adjacent_pairs"]:
            for label in d["nonadjacent_path_pair_labels"]:
                if (
                    vocabulary_params["remove_unique_words"] and word_counts[label] <= 2
                ):  # remove unique words
                    unique_word = label
                    continue
                write_labels_to_file(
                    label, "co_occurence", fh_vocabulary, fh_vocabulary_legend, word_tag
                )
                has_non_unique_words = True
                for label_copy in multiply_label(
                    vocabulary_params["non_adjacent_pairs"] - 1, label
                ):
                    write_labels_to_file(
                        label_copy,
                        "co_occurence_copy",
        ...
        if vocabulary_params["direct_edges"]:
            for label in d["direct_edge_labels"]:
                if (
                    ...
                    continue
                if not (
                    vocabulary_params["neighborhoods"]
                    and 1 in vocabulary_params["wlk_sizes"]
                    and g.degree(n) == 1
                ):
                    write_labels_to_file(
                        label,
                    ...
                    has_non_unique_words = True
                for label_copy in multiply_label(
                    vocabulary_params["direct_edges"] - 1, label
                ):
                    write_labels_to_file(
                        label_copy,
                        "co_occurence_copy",
    ...
    if vocabulary_params["root_child_relations"]:
        for label in g.nodes["0"]["root_child_relations"]:
            ...
            has_non_unique_words = True
            for label_copy in multiply_label(
                vocabulary_params["root_child_relations"] - 1, label
            ):
                write_labels_to_file(
                    label_copy,
                    "root_child_relations_copy",
                    fh_vocabulary,
                    fh_vocabulary_legend,
                    word_tag,
                )
    ...
    if vocabulary_params["mutually_exclusive_pairs"]:
        for label in mutually_exclusive_pairs[g]:
            ...
            has_non_unique_words = True
            for label_copy in multiply_label(
                vocabulary_params["mutually_exclusive_pairs"] - 1, label
            ):
                write_labels_to_file(
                    label_copy,
                    "mutually_exclusive_copy",
                    fh_vocabulary,
                    fh_vocabulary_legend,
                    word_tag,
                )
    ...
    pd.DataFrame.from_dict(data=label_tags, orient="index").to_csv(
        label_legend_filename, header=False
    )
    ...
    if vocabulary_params["remove_unique_words"] and not has_non_unique_words:
        ...
        write_labels_to_file(
            unique_word, "unique_word", fh_vocabulary, fh_vocabulary_legend, word_tag
        )
...
def encode_neighborhoods(
    graphs, node_label_attr_name, max_h, root_label=0, label_to_ignore=None
):
...
def build_tree_vocabulary(
    fnames, max_h, vocabulary_params, node_label_attr_name="Label", word_tag=""
):
    global node_label_generator
    global mutually_exclusive_pairs
    ...
    graphs = encode_neighborhoods(
        graphs,
        node_label_attr_name,
        max_h,
        root_label=vocabulary_params["root_label"],
        label_to_ignore=vocabulary_params["ignore_label"],
    )
    ...
    non_labeled_graphs = encode_neighborhoods(
        non_labeled_graphs, node_label_attr_name, max_h
    )
    ...
    for g in graphs:
        for node in g.nodes():
            ...
            for reachable_node in path_to_root[
                1:-2
            ]:  # exclude root, the current node and the ancester. The ancester will be added though the direct edges.
                reachable_node_label = get_node_label(g, reachable_node)
                if str(reachable_node_label) == str(RELABELED_LABEL_TO_IGNORE):
                    continue
                # REMOVE
                if reachable_node_label == node_label:
                    continue
    ...
    for g in graphs:
        ...
        if list_all_paths:
            longest_path = max(
                longest_path, max([len(path) for path in list_all_paths])
            )
        ...
        for path_pair in path_pairs:
            ...
            path_pair = list(set(path_pair[0]) - set(path_pair[1])), list(
                set(path_pair[1]) - set(path_pair[0])
            )
            mutual_exclusive_pairs = list(itertools.product(path_pair[0], path_pair[1]))
            for pair in mutual_exclusive_pairs:
                if str(get_node_label(g, pair[0])) == str(
                    RELABELED_LABEL_TO_IGNORE
                ) or str(get_node_label(g, pair[1])) == str(RELABELED_LABEL_TO_IGNORE):
                    continue
                ...
                if feature_1 == feature_2:
                    continue
                feature = "_".join(
                    sorted([feature_1, feature_2]) + ["mutually_exclusive"]
                )
    ...
    for fname, g in zip(fnames, graphs):
        dump_sg2vec_str(fname, g, vocabulary_params, word_tag)
    ...
    with open(os.path.join(dir_path, LOGS_FILENAME), "w") as logs_file:
        ...
        logs_file.write("\n\n")
        logs_file.write(
            "Number of trees:" + str(len(fnames) / 2)
        )  # the number of trees is doubled.
        logs_file.write("\n")
