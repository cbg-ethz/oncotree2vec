...
def compute_rank_deviation(scores_sorted_by_rank):
    ...
    current_similarity_scored = [
        item.similarity_to_reference for item in scores_sorted_by_rank
    ]

    min_rank_score = {}
    for rank in ranks:
        min_rank_score[rank] = min(
            [
                item.similarity_to_reference
                for item in scores_sorted_by_rank
                if item.rank == rank
            ]
        )
...
def get_sorted_scores(df_distances, df_vocabulary_intersections):
    ...
    for sample in prefixes:
        ...
        for tree in sample_trees:
            ...
            vocabulary_intersection_to_reference = df_vocabulary_intersections[sample][
                tree
            ]
            tree_variation = TreeSample(
                tree, distance_to_reference, vocabulary_intersection_to_reference
            )
        ...
        stdev_for_equal_scores[sample] = statistics.pstdev(
            [x.similarity_to_reference for x in scores if x.rank == max_rank]
        )

        cummulated_error_scores[sample], avg_error_scores[sample] = (
            compute_rank_deviation(scores)
        )
