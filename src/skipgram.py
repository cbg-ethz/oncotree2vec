...
class skipgram(object):
    ...
    def __init__(
        ...
        (
            ...
            self.loss,
            self.optimizer,
            self.out_path_prefix,
        ) = self.trainer_initial()

    def trainer_initial(self):
        graph = tf.Graph()
        with graph.as_default():
            batch_inputs = tf.placeholder(
                tf.int32,
                shape=(
                    [
                        None,
            ...
            graph_embeddings = tf.Variable(
                tf.random_uniform(
                    [self.num_graphs, self.embedding_size],
            ...
            batch_graph_embeddings = tf.nn.embedding_lookup(
                graph_embeddings, batch_inputs
            )  # hidden layer

            weights = tf.Variable(
                tf.truncated_normal(
                    [self.num_subgraphs, self.embedding_size],
            ...
            norm = tf.sqrt(
                tf.reduce_mean(tf.square(graph_embeddings), 1, keep_dims=True)
            )
    ...
    def train(self, corpus, batch_size):
        with tf.Session(
            ...
            min_distances = []
            sample_names = [
                corpus._id_to_sample_name[i]
                for i in range(len(corpus._id_to_sample_name))
            ]
            ...
            for i in range(self.num_steps + 1):
                ...
                while corpus.epoch_flag == False:
                    ...
                    _, loss_val, out_path_prefix = sess.run(
                        [self.optimizer, self.loss, self.out_path_prefix],
                        feed_dict=feed_dict,
                    )
                    ...
                    if step % 100 == 0:
                        if step > 0:
                            ...
                            logging.info(
                                "Epoch: %d : Average loss for step: %d : %f"
                                % (i, step, average_loss)
                            )
                    step += 1
                ...
                if i % 100 == 0:
                    df = embeddings_to_dataframe(
                        self.normalized_embeddings.eval(), sample_names, corpus
                    )
                    out_file_path_prefix = out_path_prefix + "iter" + str(i)
                    df.to_csv(out_file_path_prefix + "_embeddings.csv", index=True)
                    ...
                    (
                        ...
                        gexf_extn=self.gexf_extn,
                        generate_heatmaps=self.generate_heatmaps,
                        last_iteration=(i == self.num_steps),
                    )

                    for sample in error_percentages:
                        if sample not in error_percentages_all_iterations:
                            error_percentages_all_iterations[sample] = [
                                error_percentages[sample]
                            ]
                            avg_error_scores_all_iterations[sample] = [
                                avg_error_scores[sample]
                            ]
                            stdev_for_equal_scores_all_iterations[sample] = [
                                stdev_for_equal_scores[sample]
                            ]
                        else:
                            error_percentages_all_iterations[sample].append(
                                error_percentages[sample]
                            )
                            avg_error_scores_all_iterations[sample].append(
                                avg_error_scores[sample]
                            )
                            stdev_for_equal_scores_all_iterations[sample].append(
                                stdev_for_equal_scores[sample]
                            )
            ...
            def plot_error_scores(
                error_scores_all_iterations, filename, ylabel, y_axis_upper_limit=None
            ):
                fig = plt.figure()
                if error_scores_all_iterations:
                    ...
                    x_axis_values = np.arange(
                        100,
                        (len(error_scores_all_iterations[first_sample]) + 1) * 100,
                        100,
                    )
                        ...
                        if (
                            len(
                                set(
                                    [
                                        s.split("_")[0]
                                        for s in error_scores_all_iterations.keys()
                                    ]
                                )
                            )
                            == 1
                            ...
                            plt.plot(
                                x_axis_values,
                                error_scores_all_iterations[sample],
                                linewidth=0.5,
                            )
                        else:
                            plt.plot(
            ...
            plot_error_scores(
                error_percentages_all_iterations,
                out_path_prefix + "error_percentages.png",
                "percentages of samples with similarity score shifted from expected ordering",
            )
            plot_error_scores(
                avg_error_scores_all_iterations,
                out_path_prefix + "avg_deviation.png",
                "average similarity score deviation from expected ordering",
                y_axis_upper_limit=0.3,
            )
            plot_error_scores(
                stdev_for_equal_scores_all_iterations,
                out_path_prefix + "stdev_for_equal_scores.png",
