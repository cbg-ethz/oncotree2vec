...
def main(args):
    ...
    if os.path.isfile(treename_mapping_file):
        treename_mapping_df = pd.read_csv(
            treename_mapping_file, header=None, index_col=0, dtype=str
        ).squeeze("columns")
        treename_mapping_df.index = map(str, treename_mapping_df.index)
        treename_mapping = treename_mapping_df.to_dict()
    else:
        treename_mapping = {}
    ...
    if not dup_paths:
        ...
        for dup_path in dup_paths:
            ...
            print(sample_name, dup_sample_name)
