...
class Corpus(object):
    def __init__(
        self, corpus_folder=None, treename_mapping=None, extn="vocabulary", max_files=0
    ):
        ...
        self.extn = extn
        self.treename_mapping = treename_mapping
    ...
    def scan_corpus(self):
        ...
        for fname in self.graph_fname_list:
            subgraphs.extend(
                [l.split()[0] for l in open(fname)]
            )  # just take the first word of every sentence
        ...
        }  # output layer of the skipgram network

        self._subgraph_to_freq_map = (
        ...
        )  # total num subgraphs in all graphs
        ...
        self.subgraph_id_freq_map_as_list = (
            []
        )  # id of this list is the word id and value is the freq of word with corresponding word id
    ...
    def scan_and_load_corpus(self):
        ...
        self.graph_fname_list = get_files(
            self.corpus_folder, extn=self.extn, max_files=self.max_files
        )
        self._graph_name_to_id_map = {
            g: i for i, g in enumerate(self.graph_fname_list)
        }  # input layer of the skipgram network
        self._id_to_graph_name_map = {
            i: g for g, i in self._graph_name_to_id_map.items()
        }
        self._id_to_sample_name = {
            i: self.treename_mapping[re.search(".+/(.+)\.gexf", g).group(1)]
            for g, i in self._graph_name_to_id_map.items()
        }
        ...
        logging.info(
            "total number of subgraphs to be trained: %d" % self._subgraphcount
        )
    ...
    def generate_batch_from_file(self, batch_size):
        ...
        contextword_outputs = np.reshape(
            context_subgraph_ids, [len(context_subgraph_ids), 1]
        )
